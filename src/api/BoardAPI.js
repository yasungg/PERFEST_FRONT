...
const BoardAPI = {
  ...
  GetBoardArticle: async (communityId) => {
    return await axios.get(
      localHost + `/auth/community/getboardarticle?communityId=${communityId}`
    );
  },
  // 게시판 게시글에 좋아요 추가
  AddBoardLike: async (communityId) => {
    ...
    return await axios.post(
      localHost + `/auth/community/BoardArticle/${communityId}/addlike`,
      addLike
    );
  },
  // 게시판 작성
  BoardWrite: async (title, category, text, memberId) => {
    ...
    return await axios.post(
      localHost + `/auth/community/writeboard`,
      writeBoard
    );
  },
  // 게시판 수정
  BoardUpdate: async (title, category, text) => {
    ...
    return await axios.post(
      localHost + `/auth/community/updateboard`,
      updateBoard
    );
  },
};
